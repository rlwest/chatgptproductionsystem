...
def execute_actions(buffers, matched_productions, prodsys, original_delays):
    ...
    # Decrease the delay for pending actions and execute if delay is 0
    for i in range(len(pending_actions) - 1, -1, -1):
        prod_system_key, production, delay = pending_actions[i]
        if delay <= 1:
            production['action'](buffers)  # Corrected: Call the 'action' from production dict
...
##                print(f"Updated {buffer_key}: {str(buffers[buffer_key])}")
    ...
    # Add new actions to the pending list with their delays
    for prod_system_key, production in matched_productions:
        delay = production['action'](buffers)  # Corrected: Call the 'action' from production dict
        pending_actions.append((prod_system_key, production, delay))


# Main simulation loop remains similar.
