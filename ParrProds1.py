...
# Function to run a cycle of production rules.
def run_production_cycle(buffers, prodsys):
    print("Processing a production cycle:")
    reports = []
    production_matched_in_cycle = False

    # Shuffle the production systems
    random.shuffle(prodsys)

    for prod_system in prodsys:
...
prodsys = [productions, productions_b]

# Outer loop to run production cycle x times    
cycles = 10
